...
vector<Node*> get_neighbors(vector<vector<char>>& matrix, Node *node, int size, double g_score, double f_score = 0) {
    ...
    // Check left neighbor
    if (x > 0 && matrix[x-1][y] != '#') {
        neighbors.push_back(new Node(x-1, y, g_score+1, f_score+1, node));
    }
    ...
    // Check top neighbor
    if (y > 0 && matrix[x][y-1] != '#') {
        neighbors.push_back(new Node(x, y-1, g_score+1, f_score+1, node));
    }
...
vector<Node*> a_star(vector<vector<char>>& matrix, Node *start, Node *end, int size) {
    ...
    while (!open_set.empty()) {
        ...
        vector<Node*> neighbors = get_neighbors(matrix, current, size, current->g_score, current->f_score);
